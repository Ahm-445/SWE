            ...
            <form onSubmit={handleSubmit} className="mb-40 flex flex-col items-center" >
                <div className="align-middle flex flex-row bg-white w-90 h-50 rounded-xl shadow-lg justify-center gap-10 " >
                    ...
                    <div className="flex flex-col gap-2 justify-center ">
                        <input onChange={(e) => setForm({...form, name: e.target.value})} className=" shadow-md rounded-b-md pr-1 " type="text"></input>
                        <input onChange={(e) => setForm({...form, email: e.target.value})} dir="ltr" className=" shadow-md rounded-b-md pl-1" type="email"></input>
                        <select onChange={(e) => setForm({...form, level: parseInt(e.target.value)})} id="level" className=" shadow-md rounded-b-md pr-1" >
                        ...
                        <input onChange={(e) => setForm({...form, password: e.target.value})} dir="ltr" className=" shadow-md rounded-b-md pl-1" type="password"></input>
                    </div>
                </div>

                <div className="align-middle justify-around flex m-10 gap-10 max-w-fit">
                    <button type="submit" disabled={loading} className="lg:text-3xl lg:w-55 lg:h-15 w-60 h-10 align-middle font-medium bg-[#ab927f] text-white text-center border-none rounded-xl hover:shadow-lg ease-in duration-200" > {loading ? "جاري التسجيل..." : "تسجيل جديد"}</button>
